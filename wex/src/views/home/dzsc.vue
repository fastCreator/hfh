  ...
  <div class="dzsc">
    ...
    <div v-for="(it,i) in data" :key="i" class="goods"  @click="godetils(it)">
      ...
      <div class="right">
        ...
        <div class="jf">￥{{it.product_price}} <br/><span style="color:#ccc;font-size:12px;">赠送{{it.bonus_point}}分红积分</span></div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getList (id = 0) {
      window.server.product_list_tailored(id, (data) => {
        this.data = data.list
      })
    },
    godetils (it) {
      window.sessionStorage.sdata = JSON.stringify(it)
      this.$router.push('/page/dzscd')
    },
    addcard (it) {
      window.server.to_cart_tailored(it._id, function () {
        Toast({
          message: '添加成功',
          position: 'bottom',
          duration: 3000
